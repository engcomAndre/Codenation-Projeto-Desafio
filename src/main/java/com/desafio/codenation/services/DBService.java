...
public class DBService {
    ...
    public void instantiateTestDatabase() throws ParseException {
        ...
        int i = 0;
        Evento eventoA = new Evento("Descrição de uma evento " + ++i, logsA, TypeLevel.ERROR, "Sistema " + i, LocalDate.now(), i);
        logsA.forEach(log -> log.setEvent(eventoA));

        Evento eventoB = new Evento("Descrição de uma evento " + ++i, logsB, TypeLevel.WARNING, "Sistema " + i, LocalDate.now(), i);
        logsB.forEach(log -> log.setEvent(eventoB));

        Evento eventoC = new Evento("Descrição de uma evento " + ++i, logsC, TypeLevel.INFO, "Serviço " + i, LocalDate.now(), i);
